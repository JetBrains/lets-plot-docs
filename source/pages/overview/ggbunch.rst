...
Creates a new object - a collection of plots created with the ``ggplot`` function. Use ``add_plot()`` method to add plot to the ``bunch``. Set arbitrary location and size for plots inside the grid. To display the final ``bunch`` object, use ``show()`` method.
